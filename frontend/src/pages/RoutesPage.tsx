...
export default function RoutesPage() {
  ...
  return (<>
    <h2>路线</h2>
    <p className="hint">路线仍有袋或拒收占用时限额锁定，请先清空本路线装袋结果后再修改。</p>
    {msg && <div className="ok">{msg}</div>}
    {err && <div className="err">{err}</div>}
    <table className="table"><thead><tr><th>名称</th><th>重量上限 kg</th><th>体积上限 L</th><th>占用</th><th>操作</th></tr></thead>
    <tbody>{rows.map(r => {
      const d = draft[r.id] ?? { w: String(r.max_weight_kg), v: String(r.max_volume_l) };
      const occupied = r.bag_count > 0 || r.reject_count > 0;
      const changed = Number(d.w) !== r.max_weight_kg || Number(d.v) !== r.max_volume_l;
      const occupyParts = [
        r.bag_count > 0 ? `${r.bag_count} 袋` : "",
        r.reject_count > 0 ? `${r.reject_count} 拒收` : "",
      ].filter(Boolean);
      return <tr key={r.id}>
        <td>{r.name}</td>
        <td><input className="mono num" type="number" min="0" step="0.1" value={d.w}
          disabled={busy === r.id || occupied} onChange={e => setField(r.id, "w", e.target.value)} /></td>
        <td><input className="mono num" type="number" min="0" step="0.1" value={d.v}
          disabled={busy === r.id || occupied} onChange={e => setField(r.id, "v", e.target.value)} /></td>
        <td className="mono">{occupied ? <span className="tag tag-warn">{occupyParts.join("、")}</span> : <span className="tag">无</span>}</td>
        <td className="actions">
          <button disabled={busy === r.id || !changed || occupied}
            title={occupied ? "仍有袋或拒收占用，请先清空再修改限额" : "保存新限额"}
            onClick={() => save(r)}>保存限额</button>
          <button className="btn-danger" disabled={busy === r.id || !occupied}
